...
void DFT::DITDFT(int mode)
{
	...
	if (mode == FORWARD)
	{
		for (int i = 0; i < N; i++)
			for (int j = 0; j < N; j++)
				X[i] += x[j] * complex<Real>(cos(-pi2N*(Real)i*j), sin(-pi2N*(Real)i*j));
	}
...
void DFT::ForwardDFT()  // x[n] ==> X[k]
{
	DITDFT(FORWARD);
}
void DFT::InverseDFT() // X[k] ==> x[n]
{
	DITDFT(INVERSE);
}
